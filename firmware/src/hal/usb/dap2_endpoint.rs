...
impl DAP2Endpoint {
    ...
    fn process_rx_complete(&mut self, usb: &usb::Instance) -> Option<USBStackRequest> {
        ...
        Some(USBStackRequest::AppRequest(Request::DAP2Command((data, n))))
    }

    /// Indicate a packet has been loaded into the buffer and is ready for transmission
    fn tx_valid(&self, usb: &usb::Instance) {
        let (stat_tx, ep_type, ea) = read_reg!(usb, usb, EP3R, STAT_TX, EP_TYPE, EA);
        write_reg!(usb, usb, EP3R, CTR_RX: 1, EP_TYPE: ep_type, CTR_TX: 1, EA: ea,
                   STAT_TX: stat_valid(stat_tx));
    }
}

impl Endpoint for DAP2Endpoint {
    fn new(epbuf: &'static mut EPBuf, btable: &'static mut BTableRow) -> Self {
        DAP2Endpoint { epbuf, btable }
    }
    ...
    fn reset_endpoint(&self, usb: &usb::Instance) {
        let (stat_tx, stat_rx) = read_reg!(usb, usb, EP3R, STAT_TX, STAT_RX);
        write_reg!(usb, usb, EP3R,
    ...
    fn configure_endpoint(&self, usb: &usb::Instance) {
        ...
        let (stat_tx, stat_rx, dtog_rx, dtog_tx) =
            read_reg!(usb, usb, EP3R, STAT_TX, STAT_RX, DTOG_RX, DTOG_TX);
        write_reg!(usb, usb, EP3R,
                   CTR_RX: 1, EP_TYPE: Bulk, EP_KIND: 0, CTR_TX: 1, EA: 3,
    ...
    fn process_transfer(&mut self, usb: &usb::Instance) -> Option<USBStackRequest> {
        ...
        let (ctr_tx, ctr_rx, ep_type, ea) =
            read_reg!(usb, usb, EP3R, CTR_TX, CTR_RX, EP_TYPE, EA);
        if ctr_tx == 1 {
            // Clear CTR_TX
            write_reg!(usb, usb, EP3R,
    ...
    /// Resume reception of new bulk data
    fn rx_valid(&mut self, usb: &usb::Instance) {
        let (stat_rx, ep_type, ea) = read_reg!(usb, usb, EP3R, STAT_RX, EP_TYPE, EA);
        write_reg!(usb, usb, EP3R, CTR_RX: 1, EP_TYPE: ep_type, CTR_TX: 1, EA: ea,
                   STAT_RX: stat_valid(stat_rx));
    }

    /// Cancel reception of new bulk data
